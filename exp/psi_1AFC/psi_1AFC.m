...
%% Input device
lpt = dio_setup(psi.lpt_adr1,psi.lpt_adr2,psi.lpt_dir);
...
data.resp_btn_map = mod(str2double(ID)-1,2)+1;
...
[data.TTL_wave, data.TTL_offset] = rectpulse(psi.TTL_t,psi.TTL_V,ao.sampleRate,psi.wave_t);
...
for i = 1:size(data.seq,1);
    ...
    % Wait for key press
    [data.resp_btn(i,1),data.resp_t(i,1),data.resp_port(i,:)] = parallel_button(psi.maxRespTime,data.onset_resp{i,1},psi.resp_window,lpt);
