...
% Write header
fprintf(data_file,'ID\tage\tgender\tdate\tblock\ttrial\tstim_type\tintensity\tresp\tresp_t\tresp_btn\tstim_delay\tao_error\n');

for i = 1:length(d.seq)
   fprintf(data_file,'%s\t%s\t%s\t%s\t%.0f\t%.0f\t%.0f\t%.6f\t%.0f\t%.4f\t%.0f\t%.4f\t%.0f\n',p_data.ID,p_data.age,p_data.gender,date_str,d.block(i),i,d.seq(i,1),d.intensity(i),d.resp(i),d.resp_t(i),d.resp_btn(i),d.stim_delay(i),d.ao_error(i,1));
end
